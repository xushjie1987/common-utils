...
/**
 * A low memory footprint Cache which extends {@link LightWeightGSet}. An entry in the cache is
 * expired if (1) it is added to the cache longer than the creation-expiration period, and (2) it is
 * not accessed for the access-expiration period. When an entry is expired, it may be evicted from
 * the cache. When the size limit of the cache is set, the cache will evict the entries with
 * earliest expiration time, even if they are not expired. It is guaranteed that number of entries
 * in the cache is less than or equal to the size limit. However, It is not guaranteed that expired
 * entries are evicted from the cache. An expired entry may possibly be accessed after its
 * expiration time. In such case, the expiration time may be updated. This class does not support
 * null entry. This class is not thread safe.
 *
 * @param <K>
 *            Key type for looking up the entries
 * @param <E>
 *            Entry type, which must be (1) a subclass of K, and (2) implementing {@link Entry}
 *            interface, and
 */
public class LightWeightCache<K, E extends K> extends LightWeightGSet<K, E> {
    ...
    private static int updateRecommendedLength(int recommendedLength,
                                               int sizeLimit) {
        return sizeLimit > 0 &&
               sizeLimit < recommendedLength
                                            ? (sizeLimit / 4 * 3) // 0.75 load factor
                                            : recommendedLength;
    }
    
    /*
     * The memory footprint for java.util.PriorityQueue is low but the remove(Object) method runs in
     * linear time. We may improve it by using a balanced tree. However, we do not yet have a low
     * memory footprint balanced tree implementation.
     */
    ...
    /**
     * @param recommendedLength
     *            Recommended size of the internal array.
     * @param sizeLimit
     *            the limit of the size of the cache. The limit is disabled if it is <= 0.
     * @param creationExpirationPeriod
     *            the time period C > 0 in nanoseconds that the creation of an entry is expired if
     *            it is added to the cache longer than C.
     * @param accessExpirationPeriod
     *            the time period A >= 0 in nanoseconds that the access of an entry is expired if it
     *            is not accessed longer than A.
     */
    ...
    LightWeightCache(final int recommendedLength,
                     ...
                     final long accessExpirationPeriod,
                     final Clock clock) {
        super(updateRecommendedLength(recommendedLength,
                                      sizeLimit));
        ...
        this.queue = new PriorityQueue<Entry>(sizeLimit > 0
                                                           ? sizeLimit + 1
                                                           : 1 << 10,
    ...
    void setExpirationTime(final Entry e,
                           final long expirationPeriod) {
        e.setExpirationTime(clock.currentTime() +
                            expirationPeriod);
    }
    
    boolean isExpired(final Entry e,
                      final long now) {
    ...
    public E put(final E entry) {
        if (!(entry instanceof Entry)) {
            throw new IllegalArgumentException("!(entry instanceof Entry), entry.getClass()=" +
                                               entry.getClass());
        }
