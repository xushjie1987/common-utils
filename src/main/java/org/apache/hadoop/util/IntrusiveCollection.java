...
/**
 * Implements an intrusive doubly-linked list. An intrusive linked list is one in which the elements
 * themselves are responsible for storing the pointers to previous and next elements. This can save
 * a lot of memory if there are many elements in the list or many lists.
 */
public class IntrusiveCollection<E extends IntrusiveCollection.Element> implements Collection<E> {
    /**
     * An element contained in this list. We pass the list itself as a parameter so that elements
     * can belong to multiple lists. (The element will need to store separate prev and next pointers
     * for each.)
     */
    public interface Element {
        /**
         * Insert this element into the list. This is the first thing that will be called on the
         * element.
         */
         ...
         */
        void setPrev(IntrusiveCollection<? extends Element> list,
                     Element prev);
        
        /**
         * Set the next pointer of an element already in the list.
         */
        void setNext(IntrusiveCollection<? extends Element> list,
                     Element next);
        
        /**
         * Remove an element from the list. This is the last thing that will be called on an
         * element.
         */
    ...
    /**
     * An iterator over the intrusive collection. Currently, you can remove elements from the list
     * using #{IntrusiveIterator#remove()}, but modifying the collection in other ways during the
    ...
    private Element removeElement(Element elem) {
        ...
        prev.setNext(IntrusiveCollection.this,
                     next);
        next.setPrev(IntrusiveCollection.this,
                     prev);
        size--;
        return next;
    }
    ...
    /**
     * Get an iterator over the list. This can be used to remove elements. It is not safe to do
     * concurrent modifications from other threads while using this iterator.
     * 
     * @return The iterator.
     */
    @Override
    ...
    public boolean add(E elem) {
        ...
        prev.setNext(IntrusiveCollection.this,
                     elem);
        root.setPrev(IntrusiveCollection.this,
                     elem);
        elem.insertInternal(IntrusiveCollection.this,
                            prev,
                            root);
        size++;
        return true;
    ...
    public boolean addFirst(Element elem) {
        ...
        next.setPrev(IntrusiveCollection.this,
                     elem);
        root.setNext(IntrusiveCollection.this,
                     elem);
        elem.insertInternal(IntrusiveCollection.this,
                            root,
                            next);
