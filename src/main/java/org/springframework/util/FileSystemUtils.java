...
public abstract class FileSystemUtils {
    ...
    public static void copyRecursively(File src,
        ...
        Assert.isTrue(src != null &&
                              (src.isDirectory() || src.isFile()),
                      "Source File must denote a directory or file");
        Assert.notNull(dest,
                       "Destination File must not be null");
        doCopyRecursively(src,
                          dest);
    }
    
    /**
     * Actually copy the contents of the {@code src} file/directory to the {@code dest}
     * file/directory.
     * 
     * @param src
     *            the source directory
     ...
     *             in the case of I/O errors
     */
    private static void doCopyRecursively(File src,
                                          File dest) throws IOException {
        if (src.isDirectory()) {
            dest.mkdir();
