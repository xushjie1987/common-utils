 ...
 * <p>
 * Consider using Spring's Resource abstraction in the core package for handling all kinds of file
 * resources in a uniform manner. {@link org.springframework.core.io.ResourceLoader}'s
 * {@code getResource()} method can resolve any location to a
 * {@link org.springframework.core.io.Resource} object, which in turn allows one to obtain a
 * {@code java.io.File} in the file system through its {@code getFile()} method.
 * <p>
 * The main reason for these utility methods for resource location handling is to support
 * {@link Log4jConfigurer}, which must be able to resolve resource locations <i>before the logging
 * system has been initialized</i>. Spring's {@code Resource} abstraction in the core package, on
 * the other hand, already expects the logging system to be available.
 *
...
public abstract class ResourceUtils {
     ...
     * <p>
     * Does not check whether the URL actually exists; simply returns the URL that the given
     * location would correspond to.
     * 
     * @param resourceLocation
     *            the resource location to resolve: either a "classpath:" pseudo URL, a "file:" URL,
    ...
    public static URL getURL(String resourceLocation) throws FileNotFoundException {
        ...
        if (resourceLocation.startsWith(CLASSPATH_URL_PREFIX)) {
            ...
            if (url == null) {
                String description = "class path resource [" +
                                     path +
                                     "]";
     ...
     * system.
     * <p>
     * Does not check whether the file actually exists; simply returns the File that the given
     * location would correspond to.
     * 
     * @param resourceLocation
     *            the resource location to resolve: either a "classpath:" pseudo URL, a "file:" URL,
     ...
     *            the resource URL to resolve
     * @param description
     *            a description of the original resource that the URL was created for (for example,
     *            a class path location)
     * @return a corresponding File object
     * @throws FileNotFoundException
     *             if the URL cannot be resolved to a file in the file system
     */
     ...
     *            the resource URI to resolve
     * @param description
     *            a description of the original resource that the URI was created for (for example,
     *            a class path location)
     * @return a corresponding File object
     * @throws FileNotFoundException
     *             if the URL cannot be resolved to a file in the file system
    ...
    public static boolean isFileURL(URL url) {
        ...
        return (URL_PROTOCOL_FILE.equals(protocol) ||
                URL_PROTOCOL_VFSFILE.equals(protocol) || URL_PROTOCOL_VFS.equals(protocol));
    }
    
    /**
     * Determine whether the given URL points to a resource in a jar file, that is, has protocol
     * "jar", "zip", "vfszip" or "wsjar".
     * 
     * @param url
     *            the URL to check
    ...
    public static URL extractJarFileURL(URL jarUrl) throws MalformedURLException {
        ...
        if (separatorIndex != -1) {
            String jarFile = urlFile.substring(0,
                                               separatorIndex);
            try {
                ...
                return new URL(FILE_URL_PREFIX +
                               jarFile);
            }
        } else {
            return jarUrl;
     ...
     * <p>
     * In the case of a jar file nested within a war file, this will return a URL to the war file
    ...
    public static URL extractArchiveURL(URL jarUrl) throws MalformedURLException {
        ...
        if (endIndex != -1) {
            ...
            String warFile = urlFile.substring(0,
                                               endIndex);
